                            ...
                            <tr>
                                ...
                                <td class="align-middle text-center">
                                    <img src="{{ $umkm->logo ? asset('storage/' . $umkm->logo) : asset('images/default-logo.png') }}"
                                        alt="Logo UMKM" style="width: 60px; height: 60px; object-fit: cover; border-radius: 50%; border: 2px solid #ddd;">
                                </td>
                                <td class="align-middle">
                                    <span class="badge bg-info text-dark">{{ $umkm->jenisUsaha->nama_jenis ?? 'Tidak Ada' }}</span>
                                </td>
                                <td class="align-middle">{{ $umkm->pemilik->nama_lengkap ?? 'N/A' }}</td>
                                <td class="align-middle text-muted small">{{ Str::limit($umkm->alamat_usaha, 40) }}</td>
                                <td class="align-middle">
                                    <div class="d-grid gap-1">
                                        ...
                                        @if ($umkm->status_umkm === 'unverified')
                                            ...
                                            <form action="{{ route('admin.umkm.verify', $umkm->id) }}" method="POST">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="btn btn-success btn-sm w-100 mb-1 shadow-sm"
        ...
        <form id="tolakForm" method="POST">
            ...
            <div class="modal-content shadow-lg">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title" id="tolakModalLabel"><i class="fas fa-exclamation-triangle me-1"></i> Tolak UMKM</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="alasan_tolak" id="alasan_tolak" rows="3" class="form-control" placeholder="Jelaskan alasan penolakan secara rinci..." required></textarea>
                    </div>
                </div>
